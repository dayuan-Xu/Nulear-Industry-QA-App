...
def check_AiMessage_format():
    ...
    for key, value in vars(AiMessage).items():
        # 输出vars(AiMessage_format)中的每个键值对
        print(f"{key}: {value}")
...
def check_PromptValue_format():
    ...
    messages = prompt_template.invoke({"question":"你是谁？？", "context":"这是上下文的内容"})
...
def check_structured_output():
    ...
    structured_response = llm.invoke([HumanMessage(content="给我讲一个笑话")])
    print(structured_response)

def check_stream_output():
    # 该函数是用于测试llm的流式输出。
    for chunk in llm.stream("给我讲一个500字的笑话，主题关于“黑天鹅”"):
        #print(chunk)
